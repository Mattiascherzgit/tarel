...
def scaffold_connector(name: str, *, output: Path | None = None) -> ScaffoldResult:
    ...
    module_name = f"tarel_connector_{name.replace('-', '_')}"
...
[project]
name = "tarel-connector-{name}"
version = "0.0.1"
description = "Private {name} connector adapter for TAREL."
requires-python = ">=3.11"
dependencies = ["tarel>=0.0.2,<1"]

[project.entry-points."tarel.connectors"]
{name} = "{module_name}.connector:create_connector"

[tool.hatch.build.targets.wheel]
packages = ["{module_name}"]

[tool.hatch.build]
include = ["{module_name}/manifest.toml", "{module_name}/references/*.md"]
'''
...
- After approval, install the adapter package so its `tarel.connectors` entry point becomes active.
"""
...
not an installed or trusted connector. Start with `CONNECTOR_TASK.md`. After implementation,
private-source testing, and human review, install this directory as a normal Python package.
"""
